...
async function uploadToCloudinary(file: File, sig: UploadSignResponse): Promise<string> {
    ...
    try {
      ...
      const drinkRequests: DrinkItemRequest[] = await Promise.all(
        stagedDrinks.map(async drink => {
          ...
          try {
            const publicIds = await Promise.all(drink.files.map(f => uploadToCloudinary(f, sig)));
      ...
      const batchRes = await adminClientApi.POST('/api/admin/places/{placeId}/drinks/batch', {
