...
const RatingStars = ({ rating }: { rating: number }) => (
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map(star => (
      <Star key={star} size={10} className={cn('fill-current', star <= rating ? 'text-amber-400' : 'text-zinc-700')} />
    ))}
  </div>
);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-950 text-white font-sans selection:bg-orange-500/30 overflow-x-hidden">
      <header className="px-6 pt-12 pb-6 flex flex-col gap-1">
        <h1 className="text-5xl font-black tracking-tighter italic leading-none">YOUR LOG</h1>
            ...
            >
              <div
                className={cn(
                  ...
                  swipedId === drink.id ? 'opacity-100' : 'opacity-0 pointer-events-none'
                )}
              ...
              <motion.div
                ...
                onPointerDown={() => setSwipedId(prev => prev === drink.id ? null : (drink.id ?? null))}
              ...
              >
                <DrinkImage
                  ...
                  fallbackGradient={CATEGORY_COLORS[drink.category ?? ''] ?? 'from-zinc-700 to-zinc-900'}
                  sizes="60px"
                />

                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-white truncate">{drink.name}</h3>
                ...
                <button
                  onClick={e => { e.stopPropagation(); handleDelete(drink.id ?? ''); }}
