...
const Card = ({ children, className }: { children: React.ReactNode; className?: string }) =>
  <div className={cn('bg-zinc-900 rounded-xl p-5 border border-zinc-800/50', className)}>{children}</div>;

const FlavorRow = ({ name, score }: { name: string; score: number }) =>
  <div className="flex items-center justify-between py-2 group">
    <span className="text-zinc-300 text-sm capitalize font-medium group-hover:text-zinc-100 transition-colors">{name}</span>
    <div className="flex items-center gap-3">
      <div className="relative w-20 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
        <div className="absolute left-0 top-0 h-full bg-orange-500 rounded-full" style={{ width: `${score * 100}%` }} />
      </div>
      <span className="text-zinc-500 text-[10px] font-mono tabular-nums">{score.toFixed(1)}</span>
    </div>
  </div>;
...
export const ProfileScreen = ({ fallbackUser, fallbackTastes, fallbackStats }: Props) => {
  ...
  return <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-orange-500/30">
    <main className="flex-1 overflow-y-auto pb-32 px-6 pt-12 space-y-8">
      ...
      <Card className="grid grid-cols-3 p-0 overflow-hidden shadow-xl shadow-black/20">
        {[
          ...
          <div key={stat.label} className={cn('flex flex-col items-center justify-center py-4 px-1 text-center', idx < arr.length - 1 && 'border-r border-zinc-800')}>
            ...
            <span className="text-zinc-500 text-[10px] font-black uppercase tracking-wider">{stat.label}</span>
          </div>
        ))}
      </Card>
      ...
      <section className="space-y-2">
        <h3 className="text-zinc-400 text-[10px] font-black uppercase tracking-widest mb-4">Your Flavor Profile</h3>
      ...
      <section className="space-y-2">
        ...
        <Card className="p-0 px-5">
          {[
            ...
            <div key={row.label} className={cn('flex items-center justify-between py-4', idx < arr.length - 1 && 'border-b border-zinc-800')}>
              <span className="text-zinc-500 text-sm">{row.label}</span>
              <span className="text-zinc-300 text-sm font-medium">{row.value}</span>
            </div>
          ))}
        </Card>
      </section>
      ...
      {/* Sign out */}
      <button onClick={handleSignOut} className="w-full bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 rounded-xl py-4 flex items-center justify-center gap-3 transition-all active:scale-[0.98]">
