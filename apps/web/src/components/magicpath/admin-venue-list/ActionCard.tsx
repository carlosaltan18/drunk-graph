...
export const ActionCard: React.FC<ActionCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <div
          className={`absolute top-3 left-3 z-10 ${badgeBg} text-black text-[10px] font-black px-2 py-0.5 rounded tracking-tighter uppercase`}
        >
          {badge}
        </div>
      )}
      ...
      <div className="p-4 flex flex-col flex-grow">
        <div className="flex justify-between items-start gap-2 mb-1">
          ...
          {price && (
            <span
              className={`text-sm font-black whitespace-nowrap ${accentColor}`}
            >
              {price}
            </span>
          )}
        ...
        {accentText && (
          <div
            className={`text-[11px] font-black italic uppercase tracking-tighter mb-4 flex items-center gap-1.5 ${accentColor}`}
          >
