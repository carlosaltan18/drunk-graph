...
const TABS = [
  ...
  { id: 'browse',  label: 'Browse',  icon: LayoutGrid, href: '/dashboard/browse' },
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-zinc-950/80 backdrop-blur-xl border-t border-zinc-900 px-6 pb-8 pt-3 flex justify-between items-center z-50">
      {TABS.map(tab => {
        ...
        return (
          <Link key={tab.id} href={tab.href} className="flex flex-col items-center gap-1 transition-colors">
            <tab.icon size={20} className={cn(active ? 'text-orange-500' : 'text-zinc-600')} />
            <span className={cn('text-[10px] font-medium', active ? 'text-orange-500' : 'text-zinc-600')}>
              {tab.label}
            </span>
          </Link>
        );
