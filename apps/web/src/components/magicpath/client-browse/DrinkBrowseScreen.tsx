...
const DrinkCard = ({
  ...
  return <Link href={`/dashboard/drinks/${drink.id}`}>
    <motion.div initial={{
      opacity: 0,
      y: 10
    }} animate={{
...
export const DrinkBrowseScreen = ({ drinks: apiDrinks, initialSearch = '', initialCategory }: Props) => {
  const [activeCategory, setActiveCategory] = React.useState(initialCategory ?? 'all');
  const [searchQuery, setSearchQuery] = React.useState(initialSearch);

  const drinks: Drink[] = apiDrinks.map(d => ({
    id: d.id ?? '',
    name: d.name ?? 'Unknown',
  ...
  const filteredDrinks = drinks.filter(drink => {
    ...
    const matchesSearch = drink.name.toLowerCase().includes(searchQuery.toLowerCase()) || drink.category.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-orange-500/30 pb-32">
      ...
      <section className="mb-8">
        <div className="flex overflow-x-auto no-scrollbar gap-2 px-6">
          {CATEGORIES.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={cn("whitespace-nowrap px-5 py-2.5 rounded-full text-xs font-bold transition-all duration-200", activeCategory === category.id ? "bg-orange-500 text-black scale-105 shadow-[0_0_20px_rgba(249,115,22,0.3)]" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700")}>
              {category.label}
            </button>)}
        </div>
      </section>
      ...
      <section className="px-6">
        <div className="grid grid-cols-2 gap-4">
          <AnimatePresence mode="popLayout">
            {filteredDrinks.map(drink => <DrinkCard key={drink.id} drink={drink} />)}
          </AnimatePresence>
        </div>

        {filteredDrinks.length === 0 && <div className="py-20 text-center">
            <p className="text-zinc-500 font-medium italic">No drinks found for "{searchQuery}"</p>
          </div>}
      </section>

      {/* Pagination / Footer Info */}
      <section className="px-6 mt-12 mb-8">
        <button className="w-full bg-zinc-900 hover:bg-zinc-800 text-zinc-300 font-bold py-4 rounded-[12px] border border-zinc-800 transition-colors flex items-center justify-center gap-2 group">
          Load more
          <ChevronRight size={16} className="group-hover:translate-x-0.5 transition-transform" />
        </button>
        <p className="text-center text-zinc-600 text-[11px] mt-4 font-medium uppercase tracking-widest">
          Showing {filteredDrinks.length} of 24 drinks
        </p>
      </section>
