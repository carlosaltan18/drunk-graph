...
import { ChevronLeft, MapPin, Star, Check, ChevronRight, Info, ArrowRight } from 'lucide-react';
...
const ImageCarousel = ({
  ...
  return <section className="relative w-full h-[320px] bg-zinc-900 overflow-hidden">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
        {images.map((_, index) => <div key={index} className={cn("h-1.5 rounded-full transition-all duration-300", selectedIndex === index ? "w-6 bg-orange-500" : "w-1.5 bg-white/40")} />)}
      </div>

    </section>;
};
const FlavorIntensityDots = ({
  intensity
}: {
  intensity: number;
}) => {
...
export const DrinkDetailScreen = ({ drink, scoreFlavor = 0, scorePrice = 0, scoreFinal = 0 }: Props) => {
  ...
  return <main className="flex flex-col w-full min-h-screen bg-zinc-950 text-white font-sans selection:bg-orange-500/30 overflow-x-hidden pb-24">
      ...
      <div className="relative">
        ...
        <button onClick={() => router.back()} className="absolute top-4 left-4 z-10 w-10 h-10 rounded-full bg-zinc-900/80 flex items-center justify-center text-white backdrop-blur-sm border border-white/10 active:scale-95 transition-transform">
          <ChevronLeft size={24} />
        </button>
      </div>
      ...
      <section className="px-5 py-6 space-y-4">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <span className={cn("px-2 py-0.5 text-[10px] font-bold tracking-widest rounded", (drink?.alcoholPct ?? 0) > 0 ? "bg-orange-500/10 text-orange-400" : "bg-green-500/10 text-green-400")}>
              {(drink?.alcoholPct ?? 0) > 0 ? `${drink?.alcoholPct}% ABV` : 'NON-ALCOHOLIC'}
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-zinc-400 uppercase tracking-tight">Flavor Match</span>
                <span className="text-[10px] font-bold text-orange-500">{Math.round(scoreFlavor * 100)}%</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-zinc-400 uppercase tracking-tight">Price Fit</span>
                <span className="text-[10px] font-bold text-green-500">{Math.round(scorePrice * 100)}%</span>
              </div>
              <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                <motion.div initial={{
                width: 0
      ...
      <section className="px-5 py-4">
        <div className="bg-zinc-900 rounded-xl p-5 border border-white/5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-y-4 gap-x-8">
            {sortedFlavors.map(([name, intensity]) => <div key={name} className="flex items-center justify-between group">
            ...
            <div className="p-4 bg-zinc-900 rounded-t-3xl flex-1">
              ...
              <div className="max-w-md mx-auto px-4 space-y-8">
                ...
                <div className="flex justify-center items-center gap-2">
                  {[1, 2, 3, 4, 5].map(star => <button key={star} onClick={() => setRating(star)} className="p-1 transition-transform active:scale-90">
                      <Star size={40} className={cn("transition-colors", rating >= star ? "fill-amber-400 text-amber-400" : "text-zinc-800")} strokeWidth={1.5} />
                    </button>)}
                </div>
                ...
                <div className="pt-4">
                  <button onClick={confirmLog} className="w-full bg-orange-500 text-black font-black uppercase py-4 rounded-xl text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all">
