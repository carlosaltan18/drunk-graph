...
const FlavorSlider = ({
  ...
  return <div className="flex flex-col space-y-3 py-4 border-b border-zinc-900/50">
      <div className="flex justify-between items-end">
        ...
        <div className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold transition-colors duration-300", value > 0 ? "bg-orange-500/10 text-orange-500" : "bg-zinc-900 text-zinc-600")}>
          {value.toFixed(1)}
        </div>
      </div>
      ...
      <div className="relative h-6 flex items-center group">
        ...
        <div className="absolute w-full h-1 bg-zinc-900 rounded-full overflow-hidden">
          <motion.div className="h-full bg-orange-500" initial={{
          width: 0
        }} animate={{
          width: `${value * 100}%`
        }} transition={{
        ...
        <input type="range" min="0" max="1" step="0.1" value={value} onChange={e => onChange(parseFloat(e.target.value))} className="absolute w-full h-full opacity-0 cursor-pointer z-10" />
        
        {/* Visible Thumb */}
        <motion.div className="absolute w-4 h-4 bg-white border-2 border-orange-500 rounded-full shadow-lg pointer-events-none z-20" animate={{
...
export const FlavorProfileSetup = ({ flavors: apiFlavors }: Props) => {
  ...
  return <div className="min-h-screen w-full max-w-[402px] mx-auto bg-zinc-950 text-white font-sans overflow-x-hidden selection:bg-orange-500 selection:text-black">
      ...
      <div className="px-6 pt-12 pb-32">
        ...
        {/* Sliders Grid */}
        <motion.div className="space-y-2 mb-12" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
        delay: 0.3
          ...
          {flavors.map(flavor => <FlavorSlider key={flavor.id} flavor={flavor} value={flavorValues[flavor.id]} onChange={val => handleFlavorChange(flavor.id, val)} />)}
        </motion.div>
        ...
        <section className="mb-10">
          ...
          <div className="relative flex items-center group">
            ...
            <input type="number" placeholder="150" value={maxSpend} onChange={e => setMaxSpend(e.target.value)} className="w-full bg-zinc-900 border-none rounded-xl py-4 pl-10 pr-4 text-white font-black text-xl focus:ring-2 focus:ring-orange-500 outline-none transition-all placeholder:text-zinc-700" />
          </div>
        </section>

        {/* Preference Toggle */}
        <section className="mb-12">
          <button onClick={() => setIsNonAlcoholic(!isNonAlcoholic)} className="flex items-center justify-between w-full p-4 bg-zinc-900 rounded-xl active:scale-[0.98] transition-transform">
            <span className="text-zinc-200 font-bold text-sm tracking-tight uppercase">
              I PREFER NON-ALCOHOLIC
            </span>
            <div className={cn("w-12 h-6 rounded-full transition-colors relative", isNonAlcoholic ? "bg-orange-500" : "bg-zinc-800")}>
              <motion.div animate={{
