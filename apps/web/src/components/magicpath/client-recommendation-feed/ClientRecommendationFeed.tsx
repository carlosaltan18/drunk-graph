...
function getCategoryGradient(category: DrinkRecommendation['category']): string {
  switch (category) {
    case 'cocktail':
...
const DrinkCard = ({
  ...
  drink: DrinkRecommendation;
    ...
    <motion.article whileTap={{ scale: 0.98 }} className="group flex flex-col bg-zinc-900 border border-zinc-800 rounded-[12px] overflow-hidden">
      <DrinkImage
        src={drink.imageUrl}
        alt={drink.name}
      ...
      <div className="p-3 flex flex-col gap-1">
        ...
        <span className="text-[10px] text-orange-500 font-bold">{drink.scoreFinal}% match</span>
      </div>
    </motion.article>
  </Link>;
...
export const ClientRecommendationFeed = ({ recommendations, hasTastes }: Props) => {
  ...
  return <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-orange-500/30 pb-32">
      ...
      <header className="px-6 pt-12 pb-6">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="flex items-center justify-between mt-1">
          <p className="text-zinc-500 text-sm font-medium">Based on your flavor profile.</p>
          <Link href="/dashboard/onboarding" className="flex items-center gap-1 text-orange-500 text-xs font-bold uppercase tracking-widest hover:text-orange-400 transition-colors">
      ...
      <main className="px-6">
        <AnimatePresence mode="wait">
          {state === 'results' && <motion.section key="results" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.35 }} className="grid grid-cols-2 gap-4">
              {drinks.map(drink => <DrinkCard key={drink.id} drink={drink} />)}
            </motion.section>}

          {state === 'no-profile' && <motion.section key="no-profile" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.35 }} className="flex flex-col items-center justify-center py-20 text-center gap-8">
              <div className="flex flex-col items-center gap-4">
                ...
                <h2 className="text-3xl font-black uppercase tracking-tighter text-zinc-600 leading-tight">
                  Set your flavor<br />profile first.
                </h2>
              </div>
              <Link href="/dashboard/onboarding" className="flex items-center gap-2 px-8 py-4 bg-orange-500 text-zinc-950 font-black text-sm uppercase tracking-widest rounded-xl active:scale-95 transition-all">
                Set up tastes <ChevronRight className="w-4 h-4" />
              </Link>
            </motion.section>}

          {state === 'no-results' && <motion.section key="no-results" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.35 }} className="flex flex-col items-center justify-center py-20 text-center gap-4">
              <h2 className="text-2xl font-black uppercase tracking-tighter text-zinc-600 leading-tight max-w-xs">
                No drinks match yet —<br />more venues coming soon.
              </h2>
            </motion.section>}
        </AnimatePresence>
      </main>
