    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-8 font-mono text-sm">
      ...
      <div className="grid gap-6">
        <Section title="User session" active={!!userSession}>
          ...
          <Row
            label="role"
            value={(userSession?.user as { role?: string })?.role}
          />
        ...
        <Section title="Admin session" active={!!adminSession} admin>
          ...
          <Row
            label="role"
            value={(adminSession?.user as { role?: string })?.role}
          />
          <TokenRow
            label="fusionauth-admin JWT"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <span
          className={`font-bold ${admin ? "text-amber-400" : "text-zinc-300"}`}
        >
          {title}
        </span>
        <span
